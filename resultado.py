...
class Resultado():
    ...
    def precisao(self):
        ...
        for classe_prevista in range(len(self.mat_confusao)):
            ...
            for classe_real in range(len(self.mat_confusao)):
                soma += self.mat_confusao[classe_real][classe_prevista]
            ...
            if soma != 0: #altere o "None" para que nao ocorra divisão por zero
                self._precisao[classe_prevista] = self.mat_confusao[classe_prevista][classe_prevista]/soma #altere o "None" para o código correto
            else:
                self._precisao[classe_prevista] = "NaN"
                ##complete aqui com a foroma de assinatura correta da função warn -
                ## Deverá ser lançada uma UndefinedMetricWarning
                warnings.warn("ERROR: Div0", UndefinedMetricWarning)
        return self._precisao
    ...
    def revocacao(self):
        ...
        for classe_real in range(len(self.mat_confusao)):
            #Abaixo, navegue em self.mat_confusao para efetuar o calculo da revocação
            soma = 0
            #você pode mudar o nome "classe_b" para um nome melhor (classe_real ou classe_prevista?)
            for classe_prevista in range(len(self.mat_confusao)):
                soma += self.mat_confusao[classe_real][classe_prevista] #altere o "None" para o código correto

            #armazene no veotr a precisão para a classe, não deixe que ocorra divisão por zero
            #caso ocorra, faça um warning
            if soma != 0:#altere o "None" para que nao ocorra divisão por zero
                self._revocacao[classe_real] = self.mat_confusao[classe_real][classe_real]/soma #altere o "None" para o código correto
            else:
                self._revocacao[classe_real] = "NaN"
                ##complete aqui com a foroma de assinatura correta da função warn -
                ## Deverá ser lançada uma UndefinedMetricWarning
                warnings.warn("ERROR: Div0", UndefinedMetricWarning)
        return self._revocacao
    ...
    def f1_por_classe(self):
        ...
        for classe in range(len(self.mat_confusao)):
            ...
            if (self.mat_confusao[classe][classe]+self._precisao[classe]) == 0:
                f1[classe] = 0
            else:
                f1[classe] = 2*self._revocacao[classe]*self._precisao[classe]/(self._revocacao[classe]+self._precisao[classe])
        return f1
...
class Fold():
    ...
    def eval(self,ml_method):
        ...
        x_treino = self.df_treino.drop(columns=[self.col_classe])
        ...
        x_to_predict = self.df_data_to_predict.drop(columns=[self.col_classe])
